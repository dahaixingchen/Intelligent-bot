...
public class FileUtil {
    ...
    public static String base64ToImage(String base64, String fileName) throws IOException {
        ...
        if (cacheObject.getIsOpenOss() == 1) {
            if (StringUtils.isEmpty(fileName)) {
                fileName = getFileName();
            }
            return OssUtil.upload(base64, fileName);
        } else {
            // JDK8以上
            ...
            // 生成jpeg图片
            OutputStream out = Files.newOutputStream(Paths.get(imgUploadUrl + dayFilePatch + fileName));
            out.write(bytes);
            out.flush();
            out.close();
            String savePath = "/" + dayFilePatch + fileName;
    ...
    public static String multipartFileToBase64(MultipartFile multipartFile) throws IOException {
        ...
        // 使用Hutool进行Base64编码
        byte[] bytes = cn.hutool.core.io.FileUtil.readBytes(tempFile);
