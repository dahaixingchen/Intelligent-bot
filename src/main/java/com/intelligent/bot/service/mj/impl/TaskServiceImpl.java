...
public class TaskServiceImpl implements TaskService {
	...
	@Override
	public B<Void> submitVary(String vary,Task task, String targetMessageId, String targetMessageHash, int messageFlags) {
		String instanceId = task.getDiscordInstanceId();
		DiscordInstance discordInstance = this.discordLoadBalancer.getDiscordInstance(instanceId);
		if (discordInstance == null || !discordInstance.isAlive()) {
			throw new E("账号不可用: " + instanceId);
		}
		return discordInstance.submitTask(task, () -> discordInstance.submitVary(vary,targetMessageId, targetMessageHash,messageFlags,task.getNonce()));
	}

	@Override
	public B<Void> submitZoomOut(Integer zoomOut, Task task, String targetMessageId, String targetMessageHash, int messageFlags) {
		String instanceId = task.getDiscordInstanceId();
		DiscordInstance discordInstance = this.discordLoadBalancer.getDiscordInstance(instanceId);
		if (discordInstance == null || !discordInstance.isAlive()) {
			throw new E("账号不可用: " + instanceId);
		}
		return discordInstance.submitTask(task, () -> discordInstance.submitZoomOut(zoomOut,targetMessageId, targetMessageHash,messageFlags,task.getNonce()));
	}

	@Override
	public B<Void> submitPan(String plan, Task task, String targetMessageId, String targetMessageHash, int messageFlags) {
		String instanceId = task.getDiscordInstanceId();
		DiscordInstance discordInstance = this.discordLoadBalancer.getDiscordInstance(instanceId);
		if (discordInstance == null || !discordInstance.isAlive()) {
			throw new E("账号不可用: " + instanceId);
		}
		return discordInstance.submitTask(task, () -> discordInstance.submitPan(plan,targetMessageId, targetMessageHash,messageFlags,task.getNonce()));
	}
	...
	@Override
	public B<Void> submitShorten(Task task) {
		DiscordInstance discordInstance = this.discordLoadBalancer.chooseInstance();
		if (discordInstance == null) {
			throw new E("无可用的账号实例");
		}
		task.setDiscordInstanceId(discordInstance.getInstanceId());
		return discordInstance.submitTask(task, () -> discordInstance.shorten(task.getPrompt(),task.getNonce(),task.getId()));
	}
