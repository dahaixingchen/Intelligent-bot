...
public class DiscordServiceImpl implements DiscordService {
	...
	public B<Void> submitZoomOut(Integer zoomOut, String messageId, String messageHash, int messageFlags, String nonce) {
		String paramsStr = replaceInteractionParams(this.paramsMap.get("outPaint"), nonce)
				.replace("$message_id", messageId)
				.replace("$message_hash", messageHash)
				.replace("$zoomOut", String.valueOf(zoomOut));
		JSONObject jsonObject = JSONObject.parseObject(paramsStr);
		jsonObject.put("message_flags", messageFlags);
		paramsStr =jsonObject.toString();
		return postJsonAndCheckStatus(paramsStr);
	...
	public B<Void> submitPan(String plan, String messageId, String messageHash, int messageFlags, String nonce) {
		String paramsStr = replaceInteractionParams(this.paramsMap.get("pan"), nonce)
				.replace("$message_id", messageId)
				.replace("$message_hash", messageHash)
				.replace("$pan", plan);
		JSONObject jsonObject = JSONObject.parseObject(paramsStr);
	...
	public B<Void> shorten(String prompt,String nonce,Long id) {
		String paramsStr = replaceInteractionParams(this.paramsMap.get("shorten"), nonce)
