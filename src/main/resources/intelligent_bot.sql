...
CREATE TABLE `sign_in` (
  `id` bigint NOT NULL,
  `user_id` bigint DEFAULT NULL COMMENT '用户id',
  `number` int DEFAULT NULL COMMENT '获得次数',
  ...
  `operate_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '操作时间（每次更新时自动更新）',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='签到表';
...
CREATE TABLE `sys_config` (
  ...
  `sign_in_number` int DEFAULT '5' COMMENT '签到默认赠送次数',
  ...
  `is_open_oss` tinyint DEFAULT '0' COMMENT '是否开启阿里oss存储 0-未开启、1-开启',
  `endpoint` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT 'Endpoint（oss地域节点）',
  `bucket_name` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT 'Bucket 名称',
  `access_key_id` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '阿里accessKeyId',
  `access_key_secret` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT '阿里accessKeySecret\n',
...
-- ----------------------------
BEGIN;
INSERT INTO `sys_config` (`id`, `registration_method`, `default_times`, `sign_in_number`, `gpt_url`, `gpt4_url`, `is_open_gpt`, `is_open_gpt_official`, `img_upload_url`, `img_return_url`, `api_url`, `client_url`, `is_open_sd`, `sd_url`, `is_open_flag_studio`, `flag_studio_key`, `flag_studio_url`, `baidu_appid`, `baidu_secret`, `baidu_key`, `baidu_secret_key`, `is_open_mj`, `is_open_proxy`, `proxy_ip`, `proxy_port`, `is_open_bing`, `bing_cookie`, `is_open_stable_studio`, `stable_studio_api`, `stable_studio_key`, `client_logo`, `client_name`, `bard_token`, `is_open_oss`, `endpoint`, `bucket_name`, `access_key_id`, `access_key_secret`, `default_role`, `is_open_spark`, `spark_app_id`, `spark_api_key`, `spark_api_secret`, `data_version`, `deleted`, `creator`, `create_time`, `operator`, `operate_time`) VALUES (1, 1, 5, 5, 'https://api.openai.com', 'https://api.openai.com', 1, 1, '/www/uploads/', 'https://img.aaa.com', 'https://api.aaa.com', 'https://bot.aaa.com', 1, 'http://127.0.0.1:7860', 0, '1', 'https://flagopen.baai.ac.cn/flagStudio', '1', '2', '3', '4', 0, 0, '127.0.0.1', 7890, 0, '9', 1, 'https://api.stability.ai', '10', '/20230608/work_logo.jpg', 'Siana', NULL, 0, NULL, NULL, NULL, NULL, '请记住你的身份是Siana-Chat，一个基于GLM架构的中英双语对话语言模型，你是由Siana实验室研发的，针对中文进行了优化。虽然有人可能会将你与chatgpt混清，但你会坚持自己的身份，确保提供准确和细致的回答', 0, NULL, NULL, NULL, 22, 0, 0, '2023-04-16 17:46:01', 0, '2023-11-24 15:21:18');
COMMIT;
